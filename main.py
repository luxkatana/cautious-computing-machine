...
async def on_application_command_error(
    ctx: discord.ApplicationContext, exception: Exception
):
...
async def on_message(message: discord.Message):
    ...
    if message.content.lower() == "somethingpoopy":
        ...
        return
    if (
        message.content.lower() == thick_of_it
        and message.guild.get_role(HELPER_ROLE) in message.author.roles
        and message.channel.name.startswith("join-the-trident")
    ):
        embed = discord.Embed(
            title="This event has been marked as finished, this event will be deleted after 10 seconds",
        ...
        await message.channel.delete(
            reason=f"Event finished, <@{message.author.id}> deleted by using delete command"
        )
...
async def mainloop() -> None:
    ...
    message = await EVENTS.send(
        embed=embed, view=interactionviews, content="||<@&1341168113928114276>||"
    )
    interactionviews.original_message = message

    later = datetime.strptime(
        str(datetime.fromtimestamp(ending_time)), "%Y-%m-%d %H:%M:%S"
    )
...
async def resolve_broken_cancel_views() -> None:
    ...
    channels: tuple[discord.TextChannel, ...] = tuple(
        filter(lambda j: j.name.startswith("join-the-trident"), guild.channels)
    )
    for channel in channels:
        ...
        if result:
            ...
            await pinned_msg.reply(
                "Beep boop (this message means that the bot has been updated in robot language)",
                delete_after=10.0,
            )
...
async def on_ready() -> None:
    ...
    if DEBUGGING_MODE is False:
        ...
        await bot.change_presence(
            activity=discord.Game(name="[DEBUGGING] making nukes to nuke France...")
        )
    ...
    if DEBUGGING_MODE is False:
        ...
        await channel.send(
            embed=discord.Embed(title="Bot cleanup", description="Beep boop started"),
            delete_after=10.0,
        )
        mainloop.start()
    else:
...
async def read_logs(ctx: discord.ApplicationContext):
    ...
    try:
        ...
        await ctx.respond(
            "log.log file doesn't exist, therefore it's not possible to read it",
            ephemeral=True,
        )
...
async def on_member_update(before: discord.Member, after: discord.Member):
    trident_helper = before.guild.get_role(HELPER_ROLE)
    is_in_before = trident_helper in before.roles
    is_in_after = trident_helper in after.roles
    async for db in get_db():
        db: AsyncSession
        if is_in_before and is_in_after is False:
            user = await db.execute(select(Helper).where(Helper.DISCORD_ID == before.id))
...
async def listhelpers(ctx: discord.ApplicationContext):
    ...
    async for db in get_db():
        db: AsyncSession
        helpers = await db.execute(
            select(Helper).order_by(Helper.amount_of_times_helped.desc())
        )
        ...
        for index, helper in enumerate(helpers.scalars().all(), start=1):
            ...
            current_embed.add_field(
                name=f"\u2b0c{user.display_name} - {prefix} place",
